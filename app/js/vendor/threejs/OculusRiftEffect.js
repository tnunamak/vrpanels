...
THREE.OculusRiftEffect = function ( renderers, options ) {

    if(Array.isArray(renderers)) {
        this.separateLeftRight = true
    } else {
        renderers = [renderers];
    }
	...
	this.render = function ( scenes, camera ) {
        if(!Array.isArray(scenes)) {
            if(this.separateLeftRight) {
                throw Error('Need two scenes for two renderers');
            }
            scenes = [scenes];
        }
		...
		// Render right
		renderViewport(this.preLeftRender, scenes[0], left, renderers[0]);
		renderViewport(this.preRightRender, this.separateLeftRight ? scenes[1] : scenes[0], right, this.separateLeftRight ? renderers[1] : renderers[0]);

        function renderViewport(preRenderCallback, scene, data, renderer) {
            preRenderCallback();

            pCamera.projectionMatrix.copy(data.proj);

            // Changed matrix to matrixWorld so that parents to the camera can affect the position
            pCamera.matrix.copy(camera.matrixWorld).multiply(data.tranform);
            pCamera.matrixWorldNeedsUpdate = true;

            renderer.setViewport(data.viewport[0], data.viewport[1], data.viewport[2], data.viewport[3]);

            RTMaterial.uniforms['lensCenter'].value = data.lensCenter;
